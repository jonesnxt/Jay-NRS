...
var NRS = (function (NRS, $, undefined) {
    ...
    var tour = null;
    var path = NRS.server+"/plugins/multigateway/";
    var timer;
...
$(".hide_secret_phrase").show();
    ...
    function getDepositAddr(bIsStartSearch) {
        if (bIsStartSearch) {
            ...
            $.each(mgwAccount, function (mgwAccountIndex, mgwAccountValue) {
                $.ajax({
                    url: NRS.server+'/nxt?requestType=getBlockchainTransactions&account=' + mgwAccountValue + '&timestamp=0&type=1&subtype=0&withMessage=true',
                    dataType: 'json',
                    async: false,
                    success: function (result) {
